...
molochPlugin.prototype.start = function(callback) {
  ...
  if (this.options.print_stats){
    ...
    this.timer = setInterval(function () {
      ...
      var lastMinuteRateIN = lastMinuteCount / 60;
      var lastMinuteCount = _this.eventCount - _this.eventCountLast;
      var dropedEventCount = lastMinuteCountIN - lastMinuteCount;
      console.log('' + now + ' in queue:' + _this.eventQueue.length + ' in total: '+_this.eventCountIN+' last minute:'+lastMinuteCount+' rate:'+lastMinuteRate+'eps'+' droped:'+dropedEventCount+paused);
...
molochPlugin.prototype.send = function(event) {
  ...
  if (event && event.event && event.event.event_second) {
    ...
    _event.srcIP = ntoa(event.event.source_ip);
    _event.srcPort = event.event.source_port;
    _event.dstIP = ntoa(event.event.destination_ip);
...
molochPlugin.prototype.post = function(data) {
  //helper funcs...
  function getTagIdFromMolochDB(tag){
    return 1;
  }
  function getTagId(tag){
    if (this.tagsQueue[tag]){
      ...
      var id = getTagIdFromMolochDB(tag);
      this.tagsQueue[tag] = id;
      return id;
    }
  }
  function getSessionIDFromMolochDB(time,srcIP,srcPort,dstIP,dstPort){
    return 1;
  }
  function saveTags2Session(sessionId,tagIDs){
    return true;
  }

  this.inPost += 1;
  var sessionId = getSessionIDFromMolochDB(data.time, data.srcIP, data.srcPort, data.dstIP, data.dstPort);
  if (sessionId){
    var tagIDs = [];
    for (var i = 0; i < (data.tags.length - 1); i++) {
      tagIDs.push(getTagId(data.tags[i]); 
    }
    var ok = saveTags2Session(sessionId,tagIDs);
    if (!ok) {
      console.error('failed session tagging ;(')
    }
  } else {
    this.eventsNotFound.push(data);
    //TODO...
  }
