...
molochPlugin.prototype.start = function(callback) {
  ...
  this.stats['getIndiceName'] = 0;
  this.stats['findSessionID'] = 0;
  this.stats['findTags'] = 0;
  this.stats['updateSession'] = 0;
  this.stats['waitingForSession'] = 0;
  ...
  if (this.options.printStatstoConsole){
    ...
    this.timer = setInterval(function () {
      ...
      console.log('in post:'+_this.inPost);
  ...
  var findSessionID = function(callback){
    ...
    _this.Db.searchPrimary(_results.isIndiceName, 'session', query, function(err, result) {
      ...
      if ( result.hits.total > 0 && result.hits.hits[0] ) {
        ...
        if (data.time + _this.tcpSaveTimeout + 60 > now ){
          var future = now - (data.time + _this.tcpSaveTimeout + 60);
          console.log('event session maybe not saved yet ' + future + '' + data);
          // todo ... 
          _this.stats['waitingForSession'] +=1;
          setTimeout(function(){
            _this.stats['waitingForSession'] -=1;
            _this.post(data);
          }, future * 1000);
