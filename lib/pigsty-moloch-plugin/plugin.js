...
molochPlugin.prototype.getTagId = function(tag,cb){
    var id = this.tagsQueue.get(tag);
    if (id){
      debug('tag is in queue, good ;)');
      cb(id);
    } else {
      var _this = this;
      this.getTagIdFromMolochDB(tag, function(_id){
        debug('got tag id',_id,' for ', tag);
        _this.tagsQueue.put(tag,_id);
        debug('tags count in queue',_this.tagsQueue.size());
        cb(_id);
      });
    }
  }
...
molochPlugin.prototype.post = function(data) {
  ...
  //debug('sessions',JSON.stringify(sessions));
    if ( sessions.length > 0 ){ 
      ...
      _this.async.map(data.tags,_this.getTagId, function(err,ids){
        if (data.tags.length != ids.length) {
          debug('something wrong with tag id',data.tags, ids);
        }
        if (sessionId && ids.length > 0 ) {
          //debug('updating session:',sessionId, tagIDs);
          var ok = _this.saveTags2Session(sessionId,ids);
          if (!ok) {
