...
molochPlugin.prototype.start = function(callback) {
  ...
  if (this.options.print_stats){
    ...
    this.timer = setInterval(function () {
      ...
      var lastMinuteRateIN = lastMinuteCountIN / 60;
      var lastMinuteCount = _this.eventCount - _this.eventCountLast;
      var dropedEventCount = lastMinuteCountIN - lastMinuteCount;
      console.log('' + now + ' in queue:' + _this.eventQueue.length + ' in total: '+_this.eventCountIN+' last minute:'+lastMinuteCountIN+' rate:'+lastMinuteRateIN+'eps'+' droped:'+dropedEventCount+paused);
...
molochPlugin.prototype.post = function(data) {
  this.inPost += 1;
  var sessionId = this.getSessionIDFromMolochDB(data.time, data.srcIP, data.srcPort, data.dstIP, data.dstPort);
  if (sessionId){
    var tagIDs = [];
    for (var i = 0; i < (data.tags.length - 1); i++) {
      tagIDs.push(this.getTagId(data.tags[i])); 
    }
    var ok = this.saveTags2Session(sessionId,tagIDs);
    if (!ok) {
