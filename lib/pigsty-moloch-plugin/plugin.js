  ...
  if (event && event.event && event.event.event_second) {
    ...
    if (this.paused){
      ...
      if (needPause){
        ...
        if ( this.eventQueue.length > 10 && this.inPost < 1024) {
          var now = this.getUnixTime();
          if (( this.eventQueue[0].time + (this.options.tcpSaveTimeout + 61)/2 ) < now ) {
            //debug('good for post', new Date(this.eventQueue[0].time*1000));
            this.post(this.eventQueue.shift());
            //debug('left in queue',this.eventQueue.length);
          }
        }
      }
    }
...
molochPlugin.prototype.getSessionIDFromMolochDB = function(time,srcIP,srcPort,dstIP,dstPort,protocol,cb){
  ...
  if (ok) {
    ...
    this.Db.searchPrimary(iname, 'session', query, function(err, result) {
      ...
      _this.searchTook = (_this.searchTook + result.took) / 2; //keep average search response time
      if (err || result.error) {
...
molochPlugin.prototype.post = function(data) {
  ...
  var updateSession = function(callback) {
      ...
      /*{"startTime":1391698932918,
      "isIndiceName":"sessions-140205",
      "searchTook":620,
      "sessionID":"140205-nwMyrib4ng1B_J4ENYiDANeZ",
      "tags":[35479,35485,36533,36531],
      "saved":true}
      */
  ...
  var findSessionID = function(callback){
      ...
      _this.Db.searchPrimary(_results.isIndiceName, 'session', query, function(err, result) {
        ...
        if ( result.hits.total > 0 && result.hits.hits[0] ) {
          ...
          //_results.rawsearchresult= result;
          callback(null);
        } else {
  ...
  this.async.waterfall([getIndiceName,findSessionID,findTags.bind(this),updateSession],function(e,r){
      if (e) {
        ...
        r.taggingTime = r.endTime - r.startTime;
        debug('POST OK', JSON.stringify(r));
        debug('in post', _this.inPost);
      }
      _this.inPost -= 1; 
  })
  ...
  this.getSessionIDFromMolochDB(data.time, data.srcIP, data.srcPort, data.dstIP, data.dstPort, data.protocol, function(sessions){
    //debug('got session',JSON.stringify(sessions));
    if ( sessions && sessions.length > 0 ) { 
      _this.stats['sessionsFounded'] += 1;
      var sessionId = sessions[0]._id;
      var findTag = function (tag, cb) {
          var id = _this.tagsQueue.get(tag);
          if (id){
            //_this.tagsQueue.put(tag,{'id':id.id, 'count':id.count+1});
            cb(null,id.id);
      ...
      var updateSession =  function (err, ids) {
          debug('prep update',sessionId,ids);
      ...
      _this.async.mapSeries(data.tags, findTag, updateSession );
     } else {
