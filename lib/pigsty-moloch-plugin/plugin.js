...
molochPlugin.prototype.start = function(callback) {
  ...
  this.Db.status("sessions-*", function (err, status) {
    if (err || status.error) {
      ...
      // get earliest indicie...
      debug('ela status:', JSON.stringify(status));
    }
  }
...
molochPlugin.prototype.getSessionIDFromMolochDB = function(time,srcIP,srcPort,dstIP,dstPort,cb){
  ...
  var indices = 'sessions-' + twoDigitString(d.getUTCFullYear() % 100) + twoDigitString(d.getUTCMonth() + 1)+twoDigitString(d.getUTCDate());

  var query = {
               "from":0,
               "size":10,
               "fields" : ["_id","fp","lp","a1","p1","nodeName"],
               "query":{
                  "filtered":{
                     "query":[ 
                              {"range":{"fp":{"lte":time}}},
...
molochPlugin.prototype.post = function(data) {
    ...
    if ( sessions.length > 0 ){ 
      ...
      if ( sessions.length === 2 ) {
        sessionId = sessions[0]._id;
          ...
          debug('query',JSON.stringify(sessions[sessions.length-1]));
          for (var i = 0; i < (sessions.length - 2); i++) {
               debug('sess time:',sessions[i].fields.fp,'<', data.time,'>', sessions[i].fields.lp,sessions[i].fields.nodeName)
          }
      }
      var tagIDs = [];
  ...
  if (this.eventCount > 9998) process.exit(1);
