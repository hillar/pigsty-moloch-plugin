  ...
  if (event && event.event && event.event.event_second) {
    ...
    if (this.paused){
      ...
      if ( (now - this.options.tcpSaveTimeout) < event.event.event_second ) {
        diff = now - this.options.tcpSaveTimeout - event.event.event_second ;
        needPause = true;
...
molochPlugin.prototype.post = function(event) {
  console.dir(event.event.event_second);
};
